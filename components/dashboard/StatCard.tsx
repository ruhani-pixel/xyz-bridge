...
export function StatCard({ title, value, icon: Icon, description, trend }: StatCardProps) {
  ...
  return (
    <Card className={cn(
      "relative overflow-hidden border bg-gradient-to-br transition-all duration-500 group rounded-[1.5rem] p-5",
      ...
      <CardHeader className="flex flex-row items-center justify-between p-0 mb-4">
        <div className={cn(
          "w-10 h-10 rounded-xl flex items-center justify-center shadow-2xl transition-all duration-500 group-hover:rotate-6 group-hover:scale-110",
      ...
      <CardContent className="p-0">
        <div className="flex flex-col">
          ...
          <div className="text-3xl font-extrabold text-slate-900 tracking-tighter mb-1.5 group-hover:translate-x-1 transition-transform duration-500">{value}</div>
          <p className="text-[9px] font-bold text-slate-500 uppercase tracking-tight opacity-60 group-hover:opacity-100 transition-opacity">{description}</p>
        </div>
      </CardContent>
    </Card>
  );
